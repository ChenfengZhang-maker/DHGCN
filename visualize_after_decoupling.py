# -*- coding: utf-8 -*-
"""
DHGCN_Ultimate_Fusion.py —— 终极融合版 + t-SNE 特征空间可视化
...
class Config:
    def __init__(self):
        ...
        # 输出文件名
        self.result_csv_name = "Metrics_DHGCN_144000.csv"
...
class DHGCN_Model(nn.Module):
    def __init__(self, num_nodes, embedding_dim, hidden_dim, num_classes,
        ...
        self.se_net = SE_Net(embedding_dim)
    ...
    def forward(self, triple_idx):
        ...
        x_modes = torch.stack(mode_feats, dim=0)
        x_processed = self.imf_layer(x_modes)
        x_processed = self.se_net(x_processed)
...
# 5. 可视化函数 (t-SNE)
# =============================
def plot_tsne(embeddings, labels, filename="TSNE_Semantic_Decoupling_144000.png"):
    print(f"\nGenerating t-SNE plot... ({len(embeddings)} points)")
    ...
    plt.title("(b) t-SNE of Decoupled Semantic Space", fontsize=16, fontweight='bold')
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    ...
    model = DHGCN_Model(num_nodes, args.embedding_dim, args.hidden_dim, num_classes,
