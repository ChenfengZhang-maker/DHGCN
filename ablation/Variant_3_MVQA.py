...
class Config:
    def __init__(self):
        ...
        self.result_csv_name = os.path.join(current_dir, "Variant3_Results.csv")
...
class Variant3_MVQA(nn.Module):
    ...
    def forward(self, triple_idx):
        ...
        # 注意：这里跳过了 IMF、SE 和 HN 模块

        # --- 步骤 2: 多视角查询注意力 (MVQA) ---
        Xm = X_modes.unsqueeze(0)  # [1, M, N, D]
        Sm = self.mode_embeddings.unsqueeze(2)  # [P, M, 1, D]

        # 计算注意力权重
        attn_logits = (Xm * Sm).sum(dim=-1)  # [P, M, N]
        attn = torch.softmax(attn_logits, dim=1)  # 沿模式维度 M 归一化

        # 加权融合
        Z_per_head = (attn.unsqueeze(-1) * Xm).sum(dim=1)  # [P, N, D]
        z = Z_per_head.mean(dim=0)  # 平均池化所有视角 -> [N, D]
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    model = Variant3_MVQA(num_nodes, args.embedding_dim, args.hidden_dim, num_classes, he_list, args.num_heads,
