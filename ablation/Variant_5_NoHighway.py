...
class Config:
    def __init__(self):
        ...
        self.result_csv_name = os.path.join(current_dir, "Variant5_Results.csv")
...
# --- 模块：Channel Attention (精修 - SE-Net) ---
...
class Variant5_NoHighway(nn.Module):
    def __init__(self, num_nodes, embedding_dim, hidden_dim, num_classes, hyperedge_index_list, num_heads=4, alpha=0.7,
        ...
        # 3. 特征精修 (SE-Net)
        self.channel_attn = ChannelAttention(embedding_dim, reduction=4)

        # 注意：故意省略 HighwayGatingLayer
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    model = Variant5_NoHighway(num_nodes, args.embedding_dim, args.hidden_dim, num_classes, he_list, args.num_heads,
