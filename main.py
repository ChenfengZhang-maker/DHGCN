# -*- coding: utf-8 -*-
"""
main.py —— Official PyTorch implementation of DHGCN
(Decoupled Hypergraph Convolutional Network)

核心机制流水线：
1. Independent Convs: 提取基础特征 (IHC)
2. IMF Layer: Inter-mode feature fusion (模式间特征融合)
3. Channel Attention: 特征维度精修 (Feature Refinement - SE-Net)
4. Highway Gating: 高速公路门控 (Standard Highway Network Mechanism)
5. Multi-View Query Attention: 多视角查询注意力融合 (MVQA)
"""
...
class Config:
    def __init__(self):
        # 相对路径：请确保在 main.py 同级目录下建一个 data 文件夹，放入这三个文件
        ...
        # 输出文件名 (保存在当前运行代码的同一目录下)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        self.result_csv_name = os.path.join(current_dir, "DHGCN_Results.csv")
...
# --- A. IMF (Inter-mode feature fusion 交互) ---
...
# --- B. Channel Attention (精修 - SE-Net) ---
class ChannelAttention(nn.Module):
    def __init__(self, in_dim, reduction=4):  # 对应论文中的 r=4
    ...
    def forward(self, x):
        # x: [M, N, D]
        # Squeeze: 全局平均池化 -> [M, 1, D]
        y = x.mean(dim=1, keepdim=True)
        # Excitation: 计算通道权重 -> [M, 1, D]
        w = self.fc(y)
        # Scale: 广播乘法
        return x * w


# --- C. Highway Gating (高速公路门控 - 标准版) ---
...
class DHGCN(nn.Module):
    def __init__(self, num_nodes, embedding_dim, hidden_dim, num_classes,
                 hyperedge_index_list, num_heads=4, alpha=0.7, dropout=0.2):
        super().__init__()
        ...
        # 4. Pipeline Modules
        self.imf_layer = IMF_Layer(embedding_dim, dropout)  # Step 1: 交互 (IMF)
        self.channel_attn = ChannelAttention(embedding_dim)  # Step 2: 精修 (SE-Net)
        self.gating_layer = HighwayGatingLayer(embedding_dim)  # Step 3: 高速公路门控

        # 5. Multi-View Query Attention Params
    ...
    def forward(self, triple_idx):
        ...
        # 2. Channel Attention (精修特征 SE-Net)
        X_processed = self.channel_attn(X_processed)

        # 3. Highway Gating (高速公路门控)
        ...
        attn_logits = (Xm * Sm).sum(dim=-1)  # [4,10,16411,128]
        attn = torch.softmax(attn_logits, dim=1)  # [P, M, N]

        Z_per_head = (attn.unsqueeze(-1) * Xm).sum(dim=1)  # [P, N, D]
        z = Z_per_head.mean(dim=0)  # [N, D]
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    ...
    # 使用重命名后的 DHGCN 类
    model = DHGCN(num_nodes, args.embedding_dim, args.hidden_dim, num_classes,
                  he_list, args.num_heads, args.alpha_hgcn, args.dropout).to(args.device)
...
def main():
    ...
    print(f"\nSaved results to {args.result_csv_name}")
    print(df_res)
