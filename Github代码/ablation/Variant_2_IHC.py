...
class Config:
    def __init__(self):
        ...
        self.result_csv_name = os.path.join(current_dir, "Variant2_Results.csv")
...
class Variant2_IHC(nn.Module):
    def __init__(self, num_nodes, embedding_dim, hidden_dim, num_classes, hyperedge_index_list, alpha=0.7, dropout=0.2):
        super().__init__()
    ...
    def forward(self, triple_idx):
        ...
        # 因为没有 MVQA 或 IMF，直接简单求平均融合
        z = X_modes.mean(dim=0)  # [N, D]
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    model = Variant2_IHC(num_nodes, args.embedding_dim, args.hidden_dim, num_classes, he_list, args.alpha_hgcn,
