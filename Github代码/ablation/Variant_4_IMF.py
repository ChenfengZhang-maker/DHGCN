...
class Config:
    def __init__(self):
        ...
        self.result_csv_name = os.path.join(current_dir, "Variant4_Results.csv")
...
# 【新引入模块】：IMF (Inter-mode feature fusion 交互)
...
class Variant4_IMF(nn.Module):
    ...
    def forward(self, triple_idx):
        ...
        # (跳过了 SE-Net 特征精修 和 Highway Gating 高速公路门控)
...
def run_experiment(run_id, train_loader, val_loader, test_loader, num_nodes, num_classes, he_list, cw):
    model = Variant4_IMF(num_nodes, args.embedding_dim, args.hidden_dim, num_classes, he_list, args.num_heads,
